defmodule ExGecko.Adapter.Heroku do
  ...
  * `lines` : Number of lines to pull from logs (not applicable for `pg-backup`)
    ...
    case Porcelain.exec("heroku", porcelain_args(type, app, lines)) do
      %{status: 0, out: output} ->
        ...
        for line <- lines,
            valid?(line, type),
            into: [],
            do: line |> parse_line(type) |> drop_keys(type)
  ...
  def load_events(opts) do
    ...
    load_events(
      Map.merge(%{"app" => "brighterlink-api", "lines" => "100", "type" => "load"}, opts)
    )
  end
  ...
  def valid?(line, type) when type in ["db", "db-server"],
    do: line =~ "sample#current_transaction"
  ...
  def parse_line(line, "pg-backup") do
    [backup_id, backup_start_time, backup_end_time, backup_size, backup_db] =
      line
  ...
  def _process_metric("sample#load_avg_15m=" <> load_15m),
    do: %{"load_15m" => float_value(load_15m)}

  def _process_metric("sample#memory_total=" <> memory_total),
    do: %{"memory_total" => float_value(memory_total)}

  def _process_metric("sample#memory_rss=" <> memory_rss),
    do: %{"memory_rss" => float_value(memory_rss)}

  def _process_metric("sample#memory_cache=" <> memory_cache),
    do: %{"memory_cache" => float_value(memory_cache)}

  def _process_metric("sample#memory_swap=" <> memory_swap),
    do: %{"memory_swap" => float_value(memory_swap)}

  def _process_metric("sample#memory_pgpgin=" <> memory_pgpgin),
    do: %{"memory_pgpgin" => int_value(memory_pgpgin)}

  def _process_metric("sample#memory_pgpgout=" <> memory_pgpgout),
    do: %{"memory_pgpgout" => int_value(memory_pgpgout)}

  def _process_metric("sample#memory_quota=" <> memory_quota),
    do: %{"memory_quota" => float_value(memory_quota)}

  def _process_metric("source=" <> source), do: %{"source" => source}

  def _process_metric("sample#current_transaction=" <> current_transaction),
    do: %{"current_transaction" => current_transaction}

  def _process_metric("sample#db_size=" <> db_size), do: %{"db_size" => bytes_to_mb(db_size)}

  def _process_metric("sample#tables=" <> tables_count),
    do: %{"tables_count" => int_value(tables_count)}

  def _process_metric("sample#active-connections=" <> active_connections),
    do: %{"active_connections" => int_value(active_connections)}

  def _process_metric("sample#waiting-connections=" <> waiting_connections),
    do: %{"waiting_connections" => int_value(waiting_connections)}

  def _process_metric("sample#index-cache-hit-rate=" <> index_cache_hit_rate),
    do: %{"index_cache_hit_rate" => float_value(index_cache_hit_rate)}

  def _process_metric("sample#table-cache-hit-rate=" <> table_cache_hit_rate),
    do: %{"table_cache_hit_rate" => float_value(table_cache_hit_rate)}
  ...
  def _process_metric("sample#load-avg-15m=" <> load_15m),
    do: %{"load_15m" => float_value(load_15m)}

  def _process_metric("sample#read-iops=" <> read_iops),
    do: %{"read_iops" => int_value(read_iops)}

  def _process_metric("sample#write-iops=" <> write_iops),
    do: %{"write_iops" => int_value(write_iops)}

  def _process_metric("sample#memory-total=" <> memory_total),
    do: %{"memory_total" => kb_to_mb(memory_total)}

  def _process_metric("sample#memory-free=" <> memory_free),
    do: %{"memory_free" => kb_to_mb(memory_free)}

  def _process_metric("sample#memory-cached=" <> memory_cache),
    do: %{"memory_cache" => kb_to_mb(memory_cache)}

  def _process_metric("sample#memory-postgres=" <> memory_postgres),
    do: %{"memory_postgres" => kb_to_mb(memory_postgres)}
